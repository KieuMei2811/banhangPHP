<?php
    if(trim($_GET['tu_khoa'])!=""){
        ...
        for($i=0;$i<count($m);$i++)
        {
            ...
            if($tu!="")
            {
                $chuoi_tim_sql=$chuoi_tim_sql." names like '%".$tu."%' or";
            }
        }
        ...
        $tv = "SELECT id,names,price,photo,thuoc_menu FROM san_pham  WHERE $chuoi_tim_sql_2 ORDER BY id DESC limit $vtbd,$so_du_lieu";
        $tv_1 = mysqli_query($conn,$tv);
        echo "<table>";
